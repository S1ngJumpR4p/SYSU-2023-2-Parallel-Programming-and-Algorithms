...
vector<double> sums;    // 用来存储每个线程的求和结果
...
void* calculate_sum(void* id){
    ...
    sums[thread_id] = local_sum;    // 将线程秋和结果存储在sums中
...
int main(){
   ...
   double Len = stod(len);   // 将数字部分转换为数字
    ...
    sums.resize(THREAD_NUM);   
    /* 
    初始化sums，每个线程的求和结果刚好存储在sums的对应位置，如0号线程的求和结果存储在sums[0]，
    如果数组长度不能被线程数整除，把剩余部分的求和结果再加入到sums末尾即可
    */
    ...
    if(Length % THREAD_NUM != 0){
         // 可能数组长度未必能被THREAD_NUM整除，因此要将对剩余的(Length % THREAD_NUM)进行求和
         double temp = 0;
