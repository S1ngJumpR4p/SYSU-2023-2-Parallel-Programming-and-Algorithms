...
int M, N, K, num, rows_per_thread;   
// 矩阵的大小信息(M，N，K)、线程数量num、每个线程要计算的矩阵A的行数rows_per_thread
...
void* matrix_multiply(void* id){
    ...
    int begin_Row = thread_id * rows_per_thread, end_Row = (thread_id + 1) * rows_per_thread;
    // 根据线程号获取当前线程矩阵A需要参与运算的起始行数和终止行数
...
int main(int argc, char* argv[]){
 // 在运行时直接在最后输入参数：M，N，K，num(线程数量)
    ...
    rows_per_thread = M / THREAD_NUM;   // 计算每个线程中矩阵A有多少行要参与运算
   ...
   auto start_time = chrono::high_resolution_clock::now();     // 记录开始时间
    ...
    if(M % THREAD_NUM != 0){
        ...
        for(int i = THREAD_NUM * rows_per_thread; i < M; ++i){
            for(int k = 0; k < K; ++k){
                double temp = 0;
                for(int j = 0; j < N; ++j){
                    temp += A[i*N +j] * B[j*K + k];
