...
// 卷积
__global__ void convolution_global(float* mat, float* res, float* filter, int w_in, int h_in,
    ...
    if(x >= 0 && x < w_out && y >= 0 && y < h_out){
        ...
        for(int i = 0; i < f_h; ++i){
            for(int j = 0; j < f_w; ++j){
                sum += filter[i * f_w + j] * mat[(y * s + i) * w_in + (x * s + j)];
            }
        }
...
int main(){
 ...
 //   printf("padding OK\n");
    ...
    convolution_global<<<gridDim, blockDim>>>(r_pad, r_res, f_1, w_pad, h_pad, w_out, h_out, f_w, f_h, s);
    convolution_global<<<gridDim, blockDim>>>(g_pad, g_res, f_1+9, w_pad, h_pad, w_out, h_out, f_w, f_h, s);
    convolution_global<<<gridDim, blockDim>>>(b_pad, b_res, f_1+18, w_pad, h_pad, w_out, h_out, f_w, f_h, s);
    convolution_global<<<gridDim, blockDim>>>(r_pad, r_res, f_2, w_pad, h_pad, w_out, h_out, f_w, f_h, s);
    convolution_global<<<gridDim, blockDim>>>(g_pad, g_res, f_2+9, w_pad, h_pad, w_out, h_out, f_w, f_h, s);
    convolution_global<<<gridDim, blockDim>>>(b_pad, b_res, f_2+18, w_pad, h_pad, w_out, h_out, f_w, f_h, s);
    convolution_global<<<gridDim, blockDim>>>(r_pad, r_res, f_3, w_pad, h_pad, w_out, h_out, f_w, f_h, s);
    convolution_global<<<gridDim, blockDim>>>(g_pad, g_res, f_3+9, w_pad, h_pad, w_out, h_out, f_w, f_h, s);
    convolution_global<<<gridDim, blockDim>>>(b_pad, b_res, f_3+18, w_pad, h_pad, w_out, h_out, f_w, f_h, s);
    cudaEventRecord(end, 0);
    cudaEventSynchronize(end);
    
    float ms = 0;
    cudaEventElapsedTime(&ms, start, end);
    printf("图像大小:%d*%d;线程块维度:(%d,%d);访存方式:全局内存;所用时间:%f ms\n",h,w,blockDim_y,blockDim_x,ms);
    string output_filename = to_string(h) + "*" + to_string(w) + " " + "(" + to_string(blockDim_x) + "," +  to_string(blockDim_y) + ") sliding_window global.txt";
