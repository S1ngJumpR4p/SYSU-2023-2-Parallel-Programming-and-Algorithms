...
// 卷积
__global__ void convolution_global(float* input, float* output, float* filter, int w_out, int h_out, int f_w, int f_h){
    int y = blockIdx.y * blockDim.y + threadIdx.y, x = blockIdx.x * blockDim.x + threadIdx.x;
    if(x >= 0 && x < w_out && y >= 0 && y < h_out){
        ...
        for(int i = 0; i < f_h; ++i){
            for(int j = 0; j < f_w; ++j){
                int index_2 = i * f_w + j;
                sum += input[index_2 * (w_out * h_out) + index_1] * filter[index_2];
            }
        }
...
int main(){
    ...
    init(g, h, w);
    init(b, h, w);
 //   write_result(r, g, b, h, w, "input.txt");
    ...
    convolution_global<<<gridDim, blockDim>>>(r_col, r_res, f_1, w_out, h_out, f_w, f_h);
    convolution_global<<<gridDim, blockDim>>>(g_col, g_res, f_1+9, w_out, h_out, f_w, f_h);
    convolution_global<<<gridDim, blockDim>>>(b_col, b_res, f_1+18, w_out, h_out, f_w, f_h);
    convolution_global<<<gridDim, blockDim>>>(r_col, r_res, f_2, w_out, h_out, f_w, f_h);
    convolution_global<<<gridDim, blockDim>>>(g_col, g_res, f_2+9, w_out, h_out, f_w, f_h);
    convolution_global<<<gridDim, blockDim>>>(b_col, b_res, f_2+18, w_out, h_out, f_w, f_h);
    convolution_global<<<gridDim, blockDim>>>(r_col, r_res, f_3, w_out, h_out, f_w, f_h);
    convolution_global<<<gridDim, blockDim>>>(g_col, g_res, f_3+9, w_out, h_out, f_w, f_h);
    convolution_global<<<gridDim, blockDim>>>(b_col, b_res, f_3+18, w_out, h_out, f_w, f_h);
    cudaEventRecord(end, 0);
    cudaEventSynchronize(end);
    float ms = 0;
    cudaEventElapsedTime(&ms, start, end);
    printf("图像大小:%d*%d;线程块维度:(%d,%d);访存方式:全局内存;所用时间:%f ms\n",h,w,blockDim_y,blockDim_x,ms);
    string output_filename = to_string(h) + "*" + to_string(w) + " " + "(" + to_string(blockDim_x) + "," +  to_string(blockDim_y) + ") im2col global.txt";
