...
int main(){     
    ...
    matrix_multiply_shared<<<dimGrid, dimBlock, 2 * tile_size * (tile_size+1) * sizeof(double)>>>(A, B, C, M, N, K, tile_size);//调用核函数
    ...
    printf("Matrix C:\n");
    printMatrix(C, M, K);
    printf("线程块大小:(%d,%d) 矩阵A规模:%d*%d 矩阵B规模:%d*%d 访存方式:共享内存 所用时间:%f ms\n", blockDim_x, blockDim_y, M, N, N, K, ms);
